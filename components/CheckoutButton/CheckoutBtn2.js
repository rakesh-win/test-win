...
const CheckoutBtn2 = ({
  ...
  const proceed = () => {
    setHeadertext('Payments'),
      ...
      onShowAlert(
        'alert',
        'Please select your payment method and proceed with the payment'
      )
    try {
      if (user.name) {
        if (user.email) {
  ...
  const proceed2 = async () => {
    setHeadertext('Payments'),
      ...
      onShowAlert(
        'alert',
        "Please don't cancel or refresh the page after submit ,it may take some time to enroll."
      )
    try {
      if (user.name) {
        if (user.email) {
          if (user.mobile) {
            ...
            axios
              .post(`https://winupskill.in/api/api/checkcouponorder`, formData)
              .then(e => {
  ...
  const createorder = async () => {
    ...
    axios.post(`https://winupskill.in/api/api/orders`, formData).then(e => {
  ...
  const createorderitems = async ordid => {
    ...
    cartItems.forEach(carti => {
      ...
      axios
        .post(`https://winupskill.in/api/api/orderitems`, formData)
        .then(e => {
          //     history.push('/lesson/lesson_list')
          if (e.status == 200 || e.status == 201) {
  ...
  return (
    <div>
      ...
      <div style={{ display: price === 0 ? 'none' : 'block' }}>
        {!showButton && (
          <button className='default-btn' onClick={proceed}>
            Make Payment
          </button>
        )}
