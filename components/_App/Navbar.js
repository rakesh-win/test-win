...
import { fetchCartFromAPI } from 'store/cartAction'
...
const Navbar = ({ user, cartCount }) => {
  ...
  //   if (tempuid) {
  //     dispatch(fetchCartFromAPI(tempuid))
  //   }
  // }, [dispatch])
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (!tempuid) {
        const a = makeid(12)
        localStorage.setItem('tempuserid', a)
      }
    }
    getCourses()
    setCountry()
  ...
  useEffect(() => {
    ...
    const logUserActivities = (pageAction = 'page_load') => {
      ...
      if (locuid) {
        ...
        // Delay all logic after 5 seconds
        const timeoutId = setTimeout(() => {
          const windowLoc = window.location.href // Get the location after the delay
          ...
          try {
            navigator.sendBeacon(url, formData) // Send the beacon with the data
            console.log(`${pageAction} logged successfully`)
          } catch (error) {
            console.error('Error logging user activity:', error)
          }
        }, 5000) // Delay for 5 seconds before executing the logic
  ...
  return (
    <>
      <div
        ...
        style={{ display: hidealllinks === 'true' ? 'none' : 'block' }}
      >
        <div
          className='edemy-nav'
                ...
                <div className='others-option d-flex align-items-center'>
                  <div className='option-item'>
                    {user ? (
                      ...
                      <Link href='/authentication'>
                        <a className='default-btn-profile'>Login/Register</a>
                      </Link>
                    )}
                  </div>
                </div>
